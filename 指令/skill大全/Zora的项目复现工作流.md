...
- **在撰写文档的时候，替换成 Nuxt3 + Vue3 项目 + TypeScript + windicss 项目**
...
```text
我准备实现一个个人博客项目，项目的具体细节和详细的实现步骤都被我写在了 @docs 目录中，请读取docs目录中的文档，指导我一步一步完成整个项目。

你不必一次便完成整个项目的开发，而应该严格按照文档一步一步进行开发，每次只执行文档中的一个具体步骤，在执行完成后请给出测试该步骤完成效果的建议，并等待我完成该步骤的效果测试。

目前，我已经通过执行 `npx nuxi init zora-homepage --typescript --windicss--app --no-src-dir` 完成了项目的基础创建工作，因此，你不必再从头创建项目
```
