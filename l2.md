...
- token存储位置? 为什么要存在这些地方?
  ...
  答：token存储在vuex和本地存储，浏览器刷新的时候,vuex的数据会丢失,我们⼀般结合本地存储来解决 。

- 长token怎么用? 存储时间?

  答：用来刷新短token的,短token失效, 在响应拦截器得知token失效, 判断失效的代码, 一旦判断条件成立，发送一次刷新token的请求, 拿到最新的token, 存储到vuex内， 时效一般是7天或者14天

- 短token怎么用? 存储时间?

  答：在请求拦截器里用于发请求注入到请求头的,存储时间为2小时。
...
- 登录权限的业务逻辑是什么?
  ...
  答：首先设置一个白名单，使用路由前置导航钩子，调用本地存储里面的token，判断token是否存在，如果token存在，我们就设置免登录，其他页面都可以正常访问，如果token不存在，用户可以访问白名单里面的页面，访问其他页面则强制给跳转到登录页面。

## 3.反向代理

- 你们公司项目开发环境反向代理是怎么配置的? 生产环境要配置反向代理吗?

  答：通过伪造http请求为同源，然后将同源的请求发到反向代理服务器上，由反向代理服务器去请求真正的url，这样就绕过直接请求真正的url的跨域问题。生产环境不需要配置反向代理。
...
- 你们公司项目请求的基本路径是如何做到开发环境和生产环境配置切换的?
  ...
  答：先在package.json文件中设置开发环境和生产环境的启动配置项
  ...
  这样在启动项目时使用不同命令后通过 **process.env.NODE_ENV** 就可以判断当前处于什么环境 。然后在.evn文件中配置各个环境的请求地址，就可以实现。

## 4.请求拦截器

- 请求拦截器做了什么?

  答：判断是否存在token，如果有token，就统一注入请求头

- 响应拦截器做了什么?

  答：拦截数据异常，处理数据结构，处理token失效。
...
2. 自定义指令语法?(说一下自定义指令钩子函数和形参)
   ...
   el 指令绑定的事件可以直接用来操作dom

   binding 一个对象，里面包含了一些数据
...
3. 如何指定树形组件数据的父,子节点参数?.
   ...
   在数据中定义data:[lable为父节点：childer为子节点]
...
3. 在做路由权限的时候遇到的404页面问题, 描述下产生原因, 怎么解决的?
   ...
   - 解决方案: 删除静态路由表中的404路由配置, 手动添加到router.addRouters([...动态路由表, {404路由表}])方法
   - 内
...
4. 大屏如何做自适应的?
   ...
   初始化 var myChart = echarts.init(document.getElementById('box1'))

   样式配置 title ：标题 tooltip ：鼠标悬停气泡 xAxis : 配置横轴类别，type类型为category类别 series：销量数据，data参数与横轴一一对应，如果想调样式，也可以简单调整，比如每个条形图的颜色可以通过函数进行数组返回渲染 
...
1. ### Vue 代码层面的优化
   ...
   - ##### v-for 遍历必须为 item 添加 key. 且避免同时使用 v-if
     ...
     v-for优先级比v-if高, 会先渲染页面ll再判断显示隐藏, 所以可以在v-for外面包裹一层template标签,里面写v-if
...
## 13.项目使用nginx部署到上线( 看我发的nginx部署的md文件)

- nginx部署的代理服务器怎么配置?

- 怎么解决项目上线history的404问题?

- 部署的过程说一下

  打开软件，创建一个新的会话，安装nginx,打包公司项目

  点击新建传输, 打开本地文件和远程服务器桌面，找到etc/nginx文件夹新建一个html存放目录,把打包的dist文件夹下的代码复制到目录下，找到/etc/nginx文件夹下的nginx.conf文件，在server，配置端口号，主机名，以及项目打包的目录，使用vue官网提供的解决路由刷新404的命令，最后配置触发代理请求的代理服务器目标地址。
